...
function apiCall(_options, _callback) {
    ...
    if (Ti.Network.online && !_options.localOnly) {
        ...
        "GET" !== _options.type || _options.data || xhr.setRequestHeader("Content-Type", "text/plain");
        Ti.API.debug("[SQL REST API] apiCall send data = " + JSON.stringify(_options.data));
    ...
    switch (method) {
      case "create":
        ...
        apiCall(params, function(_response) {
            if (_response.success) {
                ...
                params.disableSaveDataLocallyOnServerError || disableSaveDataLocallyOnServerError ? logger(DEBUG, "NOTICE: The data is not being saved locally") : resp = saveData();
      ...
      case "read":
        ...
        if (!params.localOnly && (params.initFetchWithLocalData || initFetchWithLocalData)) {
            Ti.API.debug("[SQL REST API] DEBUG: reading local SQL");
            resp = readSQL();
            _.isFunction(params.success) && params.success(resp);
            model.trigger("fetch", {
                serverData: false
...
var _ = require("alloy/underscore")._, Alloy = require("alloy"), Backbone = Alloy.Backbone, moment = require("alloy/moment"), Cloud;
