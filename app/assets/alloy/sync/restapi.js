...
function apiCall (_options , _callback) {
	if (Ti.Network.online) {
		...
		for (var header in _options.headers) {
			...
			Ti.API.debug("setting requestheader " + header + " = " + JSON.stringify(_options.headers [header]));
		...
		Ti.API.debug(JSON.stringify(_options));
...
function Sync (method , model , opts) {
	...
	// Extend the provided url params with those from the model
	// config
	...
	// For older servers, emulate JSON by encoding the request
	// into an HTML-form.
	...
	// For older servers, emulate HTTP by mimicking the HTTP
	// method with `_method`
	...
	switch(method) {
		...
		case 'read':
			...
			//params.data = JSON.stringify({test:0});
...
// https://jira.appcelerator.org/browse/TIMOB-11752
